...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        self.all_items_pool = tuple(sorted(self.all_items))
        self.all_users_pool = tuple(sorted(self.all_uids.tolist()))
    ...
    def _get_neighborhood_samples(self, ids, id_str):
        ...
        all_pool = self.all_users_pool if id_str == self.config['USER_ID_FIELD'] else self.all_items_pool
        pos_ids, neg_ids = [], []
        for i in ids:
            pos_ids_my = a2a_dict[i]
            pos_ids_pool = tuple(sorted(pos_ids_my))
            if len(pos_ids_my) <= 0 or len(pos_ids_my)/len(all_set) > 0.8:
                pos_ids.append(0)
                neg_ids.append(0)
                continue
            pos_id = random.sample(pos_ids_pool, 1)[0]
            pos_ids.append(pos_id)
            neg_id = random.sample(all_pool, 1)[0]
